...
class userOperation:
    async def signUp(
        userscollection: pymongo.collection.Collection,
        name: str,
        password: str,
        email: str,
        username: str,
        ...
        f = (
            userscollection.find_one({"username": username})
            or userscollection.find_one({"email": email})
        ) != None  # flag
        if f:
    ...
    async def login(
        userscollection: pymongo.collection.Collection, email: str, password: str
    ):
        try:
            user = userscollection.find_one({"email": email})
            password = userAuth.passcrypt(password=password)
            if user["password"] == password:
                token_JWT = userAuth.genJWT(
                    username=user["username"], _id=str(user["_id"])
                )
